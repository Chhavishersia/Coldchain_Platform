...
/*
  ColdChainX – Single-file clickable prototype
  - Simple in-file "router" using route state
  - Subpages:
    FPO: Pre-cool booking, Find Reefer
    Distributor: Nudge SOP, Reroute, Raise Dispute
    Retailer: Inline Cart (Buy adds to cart)
  - Everything is mock data + client-side only
  - Includes tiny self-tests at the bottom (console.assert)
*/
...
type RouteName =
  ...
  | "dist.reroute"
  | "dist.dispute"
  | "retailer.cart"; // retained for completeness (not used in header)
...
/**
 * Helper: generate next lot ID with zero-padding.
 * Ensure we convert number → string before padStart.
 */
  ...
  return (
    <div className="flex items-center justify-between py-4">
      ...
      <div className="flex items-center gap-3">
        ...
        {/* Cart removed from global header per requirement (Retailer sees inline cart only) */}
      </div>
    </div>
  );
}
    ...
    <div className="grid gap-6">
      ...
      <div className="rounded-2xl border p-4 grid gap-3">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            <tr className="border-b"><td className="py-2">Nashik → Mumbai DC</td><td>175</td><td>High</td><td>Low</td><td><button className="px-3 py-1 rounded-xl border">Build Load</button></td></tr>
            <tr className="border-b"><td className="py-2">Nashik → Pune DC</td><td>210</td><td>Med</td><td>Med</td><td><button className="px-3 py-1 rounded-xl border">Build Load</button></td></tr>
            <tr><td className="py-2">Nashik → Nhava Sheva (Port)</td><td>180</td><td>Low</td><td>High</td><td><button className="px-3 py-1 rounded-xl border">Build Load</button></td></tr>
          </tbody>
        </table>
      </div>
  ...
  return (
    <div>
      ...
      <div className="rounded-2xl border p-4 grid gap-3 max-w-2xl">
        ...
        <div className="rounded-xl bg-gray-50 p-3 text-sm">
          Auto-attach: telemetry snippet, location, Freshness Score timeline.
        </div>
...
/* -------------------- RETAILER PAGES -------------------- */
function RetailerView({ onBuy, cart, setCart }: { onBuy: (lot: Lot) => void; cart: { lot: Lot; qty: number }[]; setCart: (v: { lot: Lot; qty: number }[]) => void }) {
  ...
  return (
    <div className="grid gap-6">
        ...
        <div className="grid md:grid-cols-3 gap-3">
          {seedLots.map((l) => (
            <div key={l.id} className="rounded-2xl border p-4 grid gap-2">
              ...
              <div className="flex gap-2">
                <button className="px-3 py-1 rounded-xl border">Gate-in QC</button>
                <button className="px-3 py-1 rounded-xl border">Fast-track</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Inline cart below retailer dashboard */}
      <div className="rounded-2xl border p-4 grid gap-3">
        <div className="flex items-center justify-between">
        ...
        <div className="rounded-2xl border p-4 grid gap-3">
          {items.map((it, idx) => (
            <div key={idx} className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2">
                <button className="px-2 py-1 rounded-lg border" onClick={()=>{
                  const copy = [...items]; copy[idx].qty = Math.max(0, copy[idx].qty - 100); setItems(copy.filter(x=>x.qty>0));
                }}>-100kg</button>
                <div className="w-24 text-right">{it.qty} kg</div>
                <button className="px-2 py-1 rounded-lg border" onClick={()=>{
                  const copy = [...items]; copy[idx].qty = copy[idx].qty + 100; setItems(copy);
                }}>+100kg</button>
              </div>
            </div>
          ))}
...
export default function App() {
  ...
  const onBuy = (lot: Lot) => {
    setCart((prev) => {
      ...
      if (idx >= 0) {
        const copy = [...prev];
        copy[idx].qty += 100; // add 100kg per click
        return copy;
      }
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      ...
      <motion.div key={`${role}-${route.name}`} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mt-4">
        ...
        {route.name === "home" && role === "Retailer" && <RetailerView onBuy={onBuy} cart={cart} setCart={setCart} />}
      ...
      <div className="mt-10 text-xs text-gray-500">
        ...
        <ul className="list-disc ml-5 space-y-1">
          <li>KPI tooltips (hover) explain how each metric is computed.</li>
          <li>Marketplace sort control (Freshness, Shelf-life, Weight).</li>
          <li>Retailer inline cart; quantity step = 100kg per click.</li>
          <li>Pre‑cool booking shows slot & queue context; Reefer page provides cost/ETA estimate and carrier choices.</li>
          <li>Trip actions open dedicated pages: Nudge (driver message), Reroute (ETA/cost deltas), Dispute (evidence + auto‑attach telemetry).</li>
        </ul>
      </div>
    </div>
  );
...
/* -------------------- Tiny self-tests (browser console) -------------------- */
(() => {
  try {
    // Test 1: ID generation pads correctly
    console.assert(nextLotId(0) === "LOT-NSK-001", "ID should be LOT-NSK-001 for count 0");
    console.assert(nextLotId(9) === "LOT-NSK-010", "ID should be LOT-NSK-010 for count 9");
    console.assert(nextLotId(123) === "LOT-NSK-124", "ID should be LOT-NSK-124 for count 123");

    // Test 2: padStart receives a string (no syntax errors)
    const n = 5; // explicit semicolon to avoid ASI pitfalls
    const s = String(n).padStart(3, "0");
    console.assert(s === "005", "padStart should work on strings");

    // Test 3: Cart add logic (dedup + increment)
    const fakeLot: Lot = { id: "LOT-NSK-999", crop: "Test", variety: "T", weight: 100, score: 90, days: 5 };
    let cart: { lot: Lot; qty: number }[] = [];
    const add = (lot: Lot) => {
      const idx = cart.findIndex((p) => p.lot.id === lot.id);
      if (idx >= 0) cart[idx].qty += 100; else cart = [...cart, { lot, qty: 100 }];
    };
    add(fakeLot); add(fakeLot);
    console.assert(cart.length === 1 && cart[0].qty === 200, "Cart should cumulate quantities in 100kg steps");

    // Test 4: nextLotId boundaries
    console.assert(nextLotId(98) === "LOT-NSK-099", "ID should be LOT-NSK-099 for count 98");
    console.assert(nextLotId(99) === "LOT-NSK-100", "ID should roll to 100 after 99");

    // Test 5: Retailer inline cart increment (local logic)
    (function testRetailerCartIncrement(){
      const lotA: Lot = { id: "LOT-NSK-777", crop: "Grape", variety: "Thompson", weight: 500, score: 90, days: 7 };
      let items: { lot: Lot; qty: number }[] = [];
      const onBuyLocal = (lot: Lot) => {
        const idx = items.findIndex((p) => p.lot.id === lot.id);
        if (idx >= 0) { items[idx].qty += 100; }
        else { items = [...items, { lot, qty: 100 }]; }
      };
      onBuyLocal(lotA); onBuyLocal(lotA);
      console.assert(items.length === 1 && items[0].qty === 200, "Retailer inline cart should cumulate to 200kg after two buys");
    })();
  } catch (e) {
    console.error("ColdChainX tests failed:", e);
  }
})();
